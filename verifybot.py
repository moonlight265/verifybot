...
ROLE_TO_REMOVE_ID = int(os.getenv('ROLE_TO_REMOVE_ID', '0'))
...
@app.route('/')
def home():
    return "✅ Бот KX Community работает!"
...
@bot.event
async def on_ready():
    print(f"✅ Бот {bot.user} успешно запущен на Render!")
    print(f"🔴 KX Community - бот верификации активен!")
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="KX Community | !verify"))
...
async def verify(ctx):
    ...
    confirm_embed = discord.Embed(description="✅ **Сообщение с верификацией отправлено!**", color=discord.Color.green())
...
async def verify_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        await ctx.send("❌ **Доступ запрещён!** Только владелец может использовать эту команду.", delete_after=5)
        await ctx.message.delete()

if __name__ == "__main__":
    print("🔴 Запуск бота KX Community на Render...")
    bot.run(TOKEN)